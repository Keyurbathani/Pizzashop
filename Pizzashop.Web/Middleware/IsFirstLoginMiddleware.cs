    ...
    public class IsFirstLoginMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                {
                    if (!path.Contains("/Auth/Login") && !path.Contains("/Auth/ForgetPassword") && !path.Contains("/Auth/ResetPassword") && !path.Contains("/Dashboard/ChangePassword") && !path.Contains("/Dashboard/Logout"))
                    {
                        _logger.LogInformation("isFirstLogin is true, redirecting to /Dashboard/ChangePassword");
                        context.Response.Redirect("/Dashboard/ChangePassword");
                        return;
                    }
                }
            }
